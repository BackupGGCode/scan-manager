...
def notSameCamera(form):
	if form.cameraR.getValue() == form.cameraL.getValue():
		return 'Cannot use the same camera for left and right sides'


def CameraBox(title,suffix,validate=NOTSET,**kargs):
	return GroupBox(name='cameraBox%s'%suffix,groupData=False,title=title,contents=[
		...
		ComboBox(name='camera%s'%suffix,label='Choose Camera:',options=lambda f:cameraOptions(getattr(f,'api%s'%suffix)),depends=['options=api%s'%suffix],required=True,validate=validate),
	],**kargs)
...
SettingsForm = TabbedForm(name='main',documentMode=True,contents=[
	Tab(name='tab1',label='Cameras',contents=[
		...
		CameraBox('Right','R',hidden=lambda f:f.mode.getValue() != Mode.V,depends='hidden=mode',validate=NOTSET),
...
class SetupWindow(BaseWidget,QtGui.QWidget):
	...
	def saveSettings(self,data):
		...
		"""
		self.app.settings.setup.data = data
	...
	def loadSettings(self):
		...
		"""
		if 'data' not in self.app.settings.setup:
			return
		self.form.setValue(self.app.settings.setup.data)
