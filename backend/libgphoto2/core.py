...
class camera(object):
    ...
    def wait_for_event(self, timeout):
        ...
        check(gp.gp_camera_wait_for_event(self._cam, timeout, PTR(eventType), PTR(eventData), context))
        
        if eventType.value in [GP_EVENT_FILE_ADDED,GP_EVENT_FOLDER_ADDED]:
            path = ctypes.cast(eventData,POINTER(CameraFilePath)).contents
            return eventType.value,(path.name,path.folder)
        else:
...
class cameraWidget(object):
    ...
    def _set_range(self, range):
        ...
        check(gp.gp_widget_set_range(self._w, f(min), f(max), f(increment)))
        return w
