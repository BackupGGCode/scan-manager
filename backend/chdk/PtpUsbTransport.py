...
class PtpUsbTransport(PtpAbstractTransport.PtpAbstractTransport):
    ...
    def __init__(self, device):
        ...
        intf = device[0][(0,0)]
        if (intf.bInterfaceClass != self.USB_CLASS_PTP):
            raise UsbException("Supplied USB device is not a PTP device")
        if (device.bDeviceClass == usb.CLASS_HUB):
            raise UsbException("Supplied USB device is not a PTP device")
        
        # Find the endpoints we need
        self.__bulkin = usb.util.find_descriptor(
        ...
        self.__bulkout = usb.util.find_descriptor(
            intf,custom_match = lambda e: 
                usb.util.endpoint_direction(e.bEndpointAddress) == usb.util.ENDPOINT_OUT and
                usb.util.endpoint_type(e.bmAttributes) == usb.util.ENDPOINT_TYPE_BULK
        )
        self.__irqin = usb.util.find_descriptor(
            intf,custom_match = lambda e: 
        ...
        # Open the USB device
        self.__usb_interface = device[0][(0,0)]
        #self.__usb_handle = device.open()
        #self.__usb_handle.setConfiguration(device[0])
        #self.__usb_handle.claimInterface(device[0][(0,0)])
    ...
    def send_ptp_request(self, request):
        ...
        if self.__bulkout.write( buffer, self.usb_write_timeout) != length:
    ...
    def send_ptp_data(self, request, data):
        ...
        # NOTE: UNTESTED CODE
        while len(buffer) > 0:
            tmp = self.__bulkout.write( buffer, self.usb_write_timeout)
            if tmp == 0:
                raise UsbException(usb.USBError)
    ...
    def findptps(cls):
        ...
        for device in usb.core.find(find_all=True):
            ...
            if device.get_active_configuration()[(0,0)].bInterfaceClass == PtpUsbTransport.USB_CLASS_PTP:
                out += (device,)
        return out
